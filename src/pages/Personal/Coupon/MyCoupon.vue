		...
		<div class="wrap">
			<header class="creditHeader" style="background-color:white;">
				...
				<tab :line-width="1" custom-bar-width="60px">
					<tab-item selected @on-item-click="onItemClick('canUseCouponList')"
						>未使用</tab-item
					>
					<tab-item @on-item-click="onItemClick('usedCouponList')">已使用</tab-item>
					<tab-item @on-item-click="onItemClick('expiredCouponList')"
						>已过期</tab-item
					>
				</tab>
			</header>
			<div class="wrapper">
				<ul class="content">
					...
					<li
						class="clearfix"
						:class="{
							gray: subType == 'usedCouponList' || subType == 'expiredCouponList'
						}"
					...
					>
						...
						>
							...
							<span class="explain">
								...
								<em v-if="item.type === '1'">
									，可叠加使用{{ item.accumulationLimit }}次
								</em>
							</span>
					...
					<li
						class="no-data"
						v-if="!list || !list[subType] || !list[subType].length"
					>
