<template>
	<div
		class="break-promise-result"
		:class="{ 'bg-no-result': loseCreditDetailList.length === 0 }"
		v-cloak
	>
		<page-header
			:title="title"
		...
		<div class="result-wrap" v-cloak>
			<div class="detail-wrap" v-show="loseCreditDetailList.length > 0">
				...
				<div class="tips">
					<span
						>！还款至嗨钱指定账户，未经嗨钱同意，您不得将借款归还到其他任何账户。</span
					>及时还款后可恢复您的信用状态，您的个人信用将不再产生任何影响！
				</div>
				<div class="swiper-container case-container">
					<div class="swiper-wrapper">
						<router-link
							class="weui-media-box weui-media-box_appmsg swiper-slide"
							:to="{
								name: 'HotNewsDetail',
								query: { id: item.id, code: 'SXAL' }
							}"
			...
			<div class="no-result" v-if="loseCreditDetailList.length === 0">
				...
				<p class="no-result-tips">
					温馨提示：按时还款，避免逾期能有效维持良好的信用记录，请珍视您的个人征信
				</p>
			</div>
			...
			<div class="pop_wrap" v-if="isShowCallDialog">
				...
				<div class="pop_main">
					<div class="pop_body">
						<p>
							尊敬的客户，由于您之前的华融消费借款已超过最后还款日期，您的逾期记录已上报<span>中国人民银行个人征信系统</span>，为了不影响您的个人信用，请及时还款。
						</p>
						详询：400-020-5566
					</div>
				...
				<div class="letter-notice-pop-main">
					...
					<mt-navbar v-model="selected" class="tab">
						<mt-tab-item
							...
							:key="index"
							@click.native="setCurrentType(detailItem)"
							>{{ detailItem.bigTitle }}</mt-tab-item
						>
					</mt-navbar>
						...
						>
							...
							>
								...
								>
									...
									<div
										slot="pagination"
										class="img-swiper-pagination swiper-pagination-clickable swiper-pagination-bullets"
									></div>
								</div>
			...
			<previewer
				v-if="isShow && this.currentType === 'SS'"
				:list="ssDetailList[0].picList"
				ref="previewer"
				:options="options"
				@on-index-change="logIndexChange"
			></previewer>
			<previewer
				v-if="isShow && this.currentType === 'ZC'"
				:list="zcDetailList[0].picList"
				ref="previewer"
				:options="options"
				@on-index-change="logIndexChange"
			></previewer>
		...
		.letter-notice-pop {
			...
			.letter-notice-pop-main {
				...
				.pic-after-text-list {
					...
					li {
						width: 100%;
						height: 0.533333rem /* 12/22.5 */;
						background: url(./images/icon_li.png) left 8px no-repeat;
						background-size: 3px 3px;
						margin-bottom: 0.311111rem /* 7/22.5 */;
						padding-left: 0.533333rem /* 12/22.5 */;
					}
				}
			}
		}
