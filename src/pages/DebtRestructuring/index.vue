<template>
	<div>
		<div class="container" v-if="showPage">
			<div class="bar"><span @click="logout">退出登录</span></div>
			<h3>诉前案件债务重组承诺书</h3>
			<p>
				本人【{{ appInfo.userName }}】，身份证号【{{ appInfo.idCard }}】，于{{
					appInfo.applyTime
				}}通过嗨钱平台申请了一笔贷款，贷款合同号【{{
					appInfo.appNo
				}}】，因个人原因未能按时还款，现经充分沟通，对相关债务重组做出如下承诺：
			</p>
			<b>第一条 债务重组计划</b>
			<p>
				如本人按照如下债务重组计划按时履行还款义务，相关债权人同意为本人减免除债务重组计划外的所有逾期金额。
			</p>
			<p>
				本次债务重组期限为{{ appInfo.period }}个月，即{{ appInfo.repayStart }}—{{
					appInfo.repayEnd
				}}。具体债务重组方案及还款计划如下：
			</p>
			...
			<b>第二条 还款方式</b>
			<p>
				本人将按时将应还款金额存入如下指定账户，并及时提供相关银行凭证，供财务核实相关金额是否到账。
			</p>
			<table border="1" cellspacing="0">
				<tr>
					<td>方式1：对公充值</td>
				...
				<tr>
					<td>
						账号名称：大同果壳金融信息服务有限公司
						开户行名称：中国民生银行股份有限公司天津白堤路支行 开 户账号：610006592
					</td>
					<td>公司名称：大同果壳金融信息服务有限公司 支付宝账号bd@guolidai.xin</td>
			...
			<b>第三条 责任追究</b>
			<p>
				如果本人再次出现违约的情况，则视为本人放弃诉讼前的和解机会，如相关债权人提起诉讼/仲裁，因此产生的一切法律后果及相关费用将由本人全部承担。
			</p>
			<div class="pic">
