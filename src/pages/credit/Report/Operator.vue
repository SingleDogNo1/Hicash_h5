	...
	<div class="operator" id="operator" ref="operator">
		...
		<div class="content" id="content" :class="{ appContent: platform === 'APP' }">
			<div class="operator-report-wrap">
				...
				<div class="score-wrap">
					<div style="width:150px;height:150px;z-index:1">
						...
						<p class="icon-verified" v-if="dataSource.isRealNameVerified === 1">
							实名认证
						</p>
				...
				<div class="desc-wrap clearfix">
					<div class="desc-item left">
						<div class="item-name">
							<i class="icon-operator"></i> <span>运营商</span>
						</div>
						<p class="item-value">{{ dataSource.name }}</p>
					</div>
					<div class="desc-item right">
						<div class="item-name">
							<i class="icon-time"></i> <span>使用时长</span>
						</div>
			...
			<div class="contacts-wrap" v-if="contactsArr.length > 0">
				...
				<div class="swiper-container">
					<div class="swiper-wrapper">
						<div
							class="swiper-slide"
							v-for="(contactsItem, index) in contactsArr"
							:key="index"
						>
			...
			<div
				...
				v-if="
					specialNum1 !== '未发现与110电话通话记录' ||
						specialNum2 !== '未找到贷款类相关号码'
				"
			>
				<h3>号码验真</h3>
				...
				<div class="report-desc">
					...
					<p class="desc">
						运营商报告通过手机号的实名验证和通信敏感信息判断，稳定的社交圈将有利于您的运营商评分
					</p>
				</div>
			</div>
			<div class="number-verification-wrap-empty" v-else>
				<h3>号码验真</h3>
				<div class="desc"><img src="./images/icon_no_problem.png" /></div>
		...
		getReportInfo() {
			...
			this.common.getCreditReport(postData).then(res => {
				if (res.data.resultCode === "1") {
					...
					this.profile.creditScore = !profile.credit_score ? 0 : profile.credit_score
					...
					contactsRegionSummary.forEach(item => {
						...
						item.percent =
							Math.round(((item.caller_count + item.callee_count) / totlaSum) * 100) /
							100
