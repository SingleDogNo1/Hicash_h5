		...
		<div class="content" :class="{ appContent: platform === 'APP' }">
			...
			<div class="year-consumption-wrap">
				...
				<div class="consumption-tips">
					...
					<p class="desc">
						年度消费情况反映您的个人消费能力。总消费、月均消费、消费次数越高，网购年龄越长，将会有利于您的还款能力评估。
					</p>
				</div>
			</div>
			<div class="consumption-trend">
				...
				<div class="consumption-tips">
					...
					<p class="desc">
						消费走势反映您消费情况的稳定性，平稳的消费走势一般表示您收入及消费水平的稳定，将有利于您的还款能力评估。
					</p>
				</div>
			</div>
			<div class="meal-record">
				<h3>壕餐记录</h3>
				<p class="title">
					<i class="line-left"></i> <span>在饿了么</span> <i class="line-right"></i>
				</p>
				<p class="desc">
					<span class="small">与</span> <span class="large">{{ shopNameSum }}</span>
			...
			<div class="most-expensive-meal-empty" v-else>
				<p class="title">今年最壕一餐是</p>
				<p class="desc">没有获取到您的记录哦， <br />快去更新报告吧！</p>
			</div>
			<div class="usually-favorite" v-if="favoriteFoodName">
				...
				<div class="favorite-food-wrap">
					<div class="favorite-food">
						...
						<p class="frequency">
							点餐次数 <span class="value">{{ favoriteFoodCount }}</span
							>次
						</p>
					</div>
				...
				this.common.wxfx(params).then(res => {
					...
					wx.ready(() => {
						wx.onMenuShareAppMessage({
							...
							link:
								this.config.NEW_MWEB_PATH +
								"/activityIntroduction?mediasource=" +
								this.mediasource,
			...
			this.common.getCreditReport(postData).then(res => {
				if (res.data.resultCode === "1") {
					...
					let thisYearSummary = monthSummary.filter(item => {
						return (
							moment(item.month).isValid() &&
							item.month.slice(0, 4) == new Date().getFullYear()
						)
					})
					let lastYearSummary = monthSummary.filter(item => {
						return (
							moment(item.month).isValid() &&
							item.month.slice(0, 4) == new Date().getFullYear() - 1
						)
					})
					...
					this.lastMonthAverage =
						lastYearSummary.length === 0 ? 0 : parseInt(this.lastTotalPriceSum / 12)
					this.yearSwitch()
					...
					let thisYearOrderList = orderList.filter(item => {
						return (
							moment(item.setup_time).isValid() &&
							item.setup_time.slice(0, 4) == new Date().getFullYear()
						)
					})
				...
				.desc-item {
					...
					.item-name {
						...
						.icon-operator {
							...
							width: 12px;
							height: 16px;
							background: url("./images/icon_operator.png") left center no-repeat;
		...
		.most-expensive-meal-empty {
			...
			.name {
				...
				i {
					...
					vertical-align: top;
					&.icon-meal-left {
						background: url("./images/icon_meal_left.png") center center no-repeat;
						background-size: cover;
					}
					&.icon-meal-right {
						background: url("./images/icon_meal_right.png") center center no-repeat;
						background-size: cover;
					}
				}
				.value {
