          ...
          <div class="third">
            ...
            <p class="price">¥{{billsDetailBySort[2].total_price}}</p>
				...
				<div class="price-tips">
					...
					<p class="desc">
						消费价格评估将您购买产品的单价进行排名，评估您的消费承受能力，单品价格越高，越有利于对您还款能力的评估。
					</p>
				</div>
			</div>
			<div class="price-wrap-empty" v-else>
				<h3>单品价格前三</h3>
				<p class="empty-desc">没有获取到您的记录哦， <br />快去更新报告吧！</p>
			</div>
		...
		getReportInfo() {
			...
			this.common.getCreditReport(postData).then(res => {
				if (res.data.resultCode === "1") {
					...
					let billsDetail = data.bills_detail.filter(item => {
						return item.status
					})
					let lastTransTime = moment(billsDetail[0].trans_time).format("YYYY-MM-DD")
					...
					this.lastMonthAverage = parseInt(this.lastTotalPriceSum / 12)
					this.yearSwitch()
