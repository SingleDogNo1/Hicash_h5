<template>
	<div>
		...
		<div class="content">
			<h2>
				<i class="icon iconfont icon-huangguan1"></i>VIP专享额度{{ kyAmount
				}}<span v-if="repayProgramData.status == 'FREZ'">(已冻结)</span>
			</h2>
			<scale
				@slideVal="slideVal"
			...
			<input
				...
				type="button"
				@click="submit"
				:value="repayProgramData.btn_str"
			/>
			<div class="vip-pro-handler">
				<p>
					每期最低本息还款：<span>￥{{ loanPayData.lowPay }}</span>
				</p>
				<p>
					月利率：<span>{{ loanPayData.lowRate }}-{{ loanPayData.highRate }}</span>
				</p>
				<router-link
					:to="{
						name: 'VipDetails',
						query: { vipMoneyVal: vipMoneyVal / 100, currentIndex: currentIndex }
					}"
					><div class="vip-cal">
						<i class="icon iconfont icon-jisuanqi"></i>费率计算器
					</div></router-link
				>
			</div>
		</div>
		...
		<div class="vipMask" v-if="isShowVipMask">
			<div class="inner">
				<div class="header">
					...
					<p><i class="iconfont">&#xe684;</i>您已成为嗨钱网VIP</p>
				</div>
				<div class="content">
					<h3>VIP可以享受以下优待</h3>
					<div class="text_list">
		...
		<div class="vipMask no-vip" v-if="isNoVip">
			<div class="inner">
				<div class="header">{{ noVipTitleVal }}</div>
				<div class="content">{{ noVipContentVal }}</div>
				<div class="footer">
					<router-link :to="{ name: 'Home' }"
						><input type="button" value="首页" v-if="!isZhunVip"
					/></router-link>
					<router-link :to="{ name: noVipLink }"
						><input type="button" :value="noVipValue" v-if="!isZhunVip"
					/></router-link>
					<input
						v-if="isZhunVip"
						class="w100"
...
export default {
	...
	methods: {
		...
		userInfo() {
			...
			this.common.UserInfo(data).then(res => {
		...
		fastLoanFirst() {
			...
			this.common.fastLoanFirst(params).then(res => {
				let data = res.data
				let userName = this.utils.getCookie("userName")
				this.$vux.loading.hide()
				if (data.resultCode == "1") {
					...
					this.utils.setCookie(
						"prodetailInfo",
						this.vipMoneyVal + ":" + this.loanProduct
					)
					this.utils.setCookie("checkSupportIsCredit", this.checkSupportIsCredit)
					this.$router.push({
						name: "Auth",
						query: { appFlowNo: data.appFlowNo, industryCode: "VIPD" }
					})
				} else {
					this.$vux.toast.text("临时订单创建失败", "middle")
				}
		...
		submit() {
			...
			if (
				this.isVip &&
				this.repayProgramData.status == "NOML" &&
				this.repayProgramData.iscredit != "0"
			) {
				//正常VIP
...
.VipDai {
	...
	.vipMask .inner .header {
		width: 100%;
		height: 3.05rem;
		color: #fff;
		border-radius: 5px 5px 0 0;
		background: -webkit-linear-gradient(#2e3244, #121212);
		background: linear-gradient(#2e3244, #121212);
		color: #fff;
	}
	.vipMask .inner .header span {
		display: block;
		text-align: center;
		font-size: 0.65rem;
		padding: 0.3rem 0 0.2rem;
	}
	.vipMask .inner .header p {
		font-size: 0.65rem;
		text-align: center;
		position: relative;
		line-height: 0.9rem;
	...
	.vipMask .inner .content {
		width: 100%;
		padding: 0.875rem 0 0.5rem 0;
	}
	.vipMask .inner .content h3 {
		font-size: 0.65rem;
		color: #cfae68;
		text-align: center;
		padding-bottom: 0.5rem;
	}
	.vipMask .inner .content .text_list {
		margin-left: 1rem;
	}
	.vipMask .inner .content .text_list p {
		color: #a4a4a4;
		text-align: left;
		font-size: 0.55rem;
		line-height: 0.95rem;
	...
	.vipMask .inner .content .text_list p strong {
		display: block;
		color: #3f3f3f;
		position: relative;
	}
	.vipMask .inner .content .text_list p strong:before {
		position: absolute;
		content: "";
		width: 0.5rem;
		height: 0.5rem;
	...
	.vipMask .inner .footer {
		width: 100%;
		text-align: center;
		min-width: auto;
		margin-bottom: 20px;
	}
	.vipMask .inner .footer button {
		width: 5.05rem;
		height: 1.75rem;
	...
	.vipMask .inner .footer button.active {
		width: 11.55rem;
		border: 1px solid #a28362;
		color: #e1d9bf;
		background: #a28362;
	}
	.vipMask .inner .footer button:hover {
		background: #81674b;
