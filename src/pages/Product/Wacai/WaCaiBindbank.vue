    ...
    ></page-header> -->
				...
				<div class="popup">
					<p class="title">
						<x-icon
							type="ios-arrow-left"
							size="24"
							@click.native="chooseCardShow = false"
						></x-icon>
						<span>选择银行卡</span>
					</p>
					<ul>
						<li
							v-for="(item, index) in chooseBankList"
							:key="index"
							@click="chooseCard(item)"
						>
							<p>{{ item.cardInfo }}</p>
						</li>
						<li class="add-card" @click="addCard"><span>新增银行卡</span> <i></i></li>
					</ul>
				</div>
			</popup>
		</div>
		...
		>
			<div class="msg-content">
				...
				<p>
					姓名： <span class="gray">{{ realName }}</span>
					<span class="orange action" @click="showModifyNameInput">{{
						modifyNameActionText
					}}</span>
				</p>
				<input
					type="text"
		...
		<confirm
			v-model="showSinaPayPop"
			title="提示"
			@on-confirm="ifJumpSinaPay(jumpSinaPayData)"
		>
...
export default {
	...
	methods: {
		...
		submitBindCardInfo(data) {
			common.submitBindCardInfo(data).then(res => {
