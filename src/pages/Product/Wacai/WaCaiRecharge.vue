<template>
  <div v-cloak>
    ...
    <div
      ...
      :style="{
				'padding-top':
					this.platform == 'APP' ? 0 : '2.26667rem'
			}"
    >
      <dd>当前应还总金额</dd>
      <group>
      ...
      <group>
        ...
        <cell title="优惠" class="coupon-cell">
          ...
          <div v-else style="display: flex; align-items: center;" @click="jumpToCoupon">
    ...
    <div class="use-balance">
      <check-icon :value.sync="isUseBalance" :class="{'disabled': isUseBalanceDisabled}">使用余额</check-icon>
      <!--<checker
...
export default {
  ...
  data() {
    return {
      ...
      couponRuleId: this.$route.query.couponRuleId ? this.$route.query.couponRuleId : ""
    };
  },
  mounted() {
  ...
  methods: {
      ...
      RechargePay.applyRecharge(applyRechargeObj).then( res=> {
        ...
        if(data.resultCode === "1"|| data.resultCode === "6014"){
          ...
          if(orderTimestamp && orderTimestamp != ""){
            let orderTimestampFormat = new Date(orderTimestamp.substring(0,4)+"-"+orderTimestamp.substring(4,6)+"-"+orderTimestamp.substring(6,8)+" 00:00:00").getTime();
    ...
    jumpToCoupon() {
      ...
      this.$router.push({ name: "Coupon", query: {userName: this.userName, rechargeAmount: rechargeAmount, source: "1", appNo: this.appNo, oldCurrentAmt: this.utils.getCookie("oldCurrentAmt")}})
    }
  },
  watch: {
