	...
	<div class="auth-content">
		...
		<section class="creditMain">
			<div class="content front">
				...
				<div class="cardImg">
					<div
						@click="uploadFileInput('ZL02')"
						v-if="!idCardInfo[0].bigPath"
						class="idcard-frontal"
					></div>
				...
				<ul class="first">
					...
					<li class="idCardNo">
						...
						<span class="val">{{
							idCardInfo[0].faceResult.idcard_number.result
						}}</span>
					</li>
				</ul>
			</div>
			<div class="content opposite">
				...
				<div class="cardImg">
					<div
						@click="uploadFileInput('ZL03')"
						v-if="!idCardInfo[1].bigPath"
						class="idcard-reverse"
					></div>
				...
				<ul class="second">
					<li class="idCardTime">
						...
						<span class="val" v-if="idCardInfo[1].faceResult.valid_date_start.result"
							>{{ idCardInfo[1].faceResult.valid_date_start.result }}-{{
								idCardInfo[1].faceResult.valid_date_end.result
							}}</span
						>
					</li>
				</ul>
		...
		>
			<div class="video-wrap">
				<div class="v-content">
					...
					<div class="first">
						<div class="v-img"><img src="./images/selfTimer.png" alt /></div>
						<div class="v-tips">
							<div class="tipContent">
								<div>请使用 <span>前置摄像头</span></div>
								<div>请用 <span>普通话</span> 读 <span>一遍</span></div>
								<div>视频时长 <span>3-5秒</span></div>
							</div>
						</div>
					</div>
					<div class="v-num">
				...
				<div class="v-notice">
					<span>查看&nbsp; <span @click="videoIsShow">拍摄须知</span> </span>
				</div>
				...
				<div class="exit"><span @click="exit">退出验证</span></div>
			</div>
		</Popup>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		uploadFileInput: function(type) {
			...
			if (type == "video") {
				...
				this.flag2 = false
				this.action =
					"/HicashAppService/VerifyVideo?bizNo=" +
					this.bizNo +
					"&userName=" +
					this.utils.getCookie("userName") +
					"&token=" +
					this.token +
					"&uuid=0c8297d7-6d3a-46da-b782-0df2434f88b" +
					"&randomNum=" +
					this.videoNumber +
					"&tempAppNo=" +
					this.utils.getCookie("appFlowNo").split(":")[1]
				this.$refs.fileInput.click()
			} else {
				this.action =
					"/HicashAppService/UploadAppPic?imgType=" +
					this.uploadType +
					"&userName=" +
					this.utils.getCookie("userName") +
					"&tempAppNo=" +
					this.utils.getCookie("appFlowNo").split(":")[1] +
					"&uploadType=HTML5&uuid=0c8297d7-6d3a-46da-b782-0df2434f88b"
			}
		...
		complete(response) {
			...
			if (response.resultCode == "50608") {
				...
				if (this.uploadType == "video") {
					...
					updateCustCardrData.append(
						"permanentAddressRaod",
						this.idCardInfo[0].faceResult.address.result
					) //string	户籍详细地址
					updateCustCardrData.append(
						"nation",
						this.idCardInfo[0].faceResult.nationality.result
					) //string	民族
					updateCustCardrData.append(
						"idCardValStartDate",
						this.idCardInfo[1].faceResult.valid_date_start.result
					) //string	身份证有效期开始时间
					updateCustCardrData.append(
						"idCardValEndDate",
						this.idCardInfo[1].faceResult.valid_date_end.result
					) //string	身份证有效期结束时间
					updateCustCardrData.append(
						"identityNo",
						this.idCardInfo[0].faceResult.idcard_number.result
					) //string	身份证号码
					updateCustCardrData.append(
						"idcardFrom",
						this.idCardInfo[1].faceResult.issued_by.result
					) //string	签发机关
					updateCustCardrData.append(
						"name",
						this.idCardInfo[0].faceResult.name.result
					) //string	真实姓名
		...
		UpdateTempAppInfo(params) {
			this.common.UpdateTempAppInfo(params).then(res => {})
		},
		GetRandomNumber(params) {
			this.common.GetRandomNumber(params).then(res => {
		...
		UpdateCustCard(params) {
			this.common.UpdateCustCard(params).then(res => {
		...
		updatePicStatus(params) {
			this.common.updatePicStatus(params).then(res => {
				let data = res.data
				if (data.resultCode == "1") {
					...
					if (
						this.utils.getCookie("industryCode") == "VIPD" &&
						this.utils.getCookie("checkSupportIsCredit") == "1"
					) {
						this.queryFirstExamineSuc()
					} else if (industryCode == "DDCP" && hqljb == "1") {
						window.location =
							this.config.MWEB_PATH + "newweb/creditInfo/editablePage.html?back=1"
					} else if (custType == "KHL2") {
						window.location =
							this.config.MWEB_PATH + "newweb/creditInfo/collarBaseInfo.html"
					} else {
						window.location =
							this.config.MWEB_PATH + "newweb/creditInfo/stuBaseInfo.html"
					}
				} else {
		...
		queryFirstExamineSuc() {
			//预审
			var params = new URLSearchParams()
			params.append("tokenKey", this.utils.uuid())
			params.append(
				"applyAmout",
				this.utils.getCookie("prodetailInfo").split(":")[0]
			)
			...
			this.common.queryFirstExamineSuc(params).then(res => {
				let data = res.data
				if (data.resultCode == "1") {
		...
		queryUserTip(queryUserTipData) {
			//预审
			this.common.queryUserTip(queryUserTipData).then(res => {
	...
	mounted() {
		...
		updateTempAppInfoData.append(
			"industryCode",
			this.utils.getCookie("industryCode")
		)
