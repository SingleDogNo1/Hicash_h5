	...
	<div class="content">
		...
		<div class="reminder-content" :class="{ appContent: platform === 'APP' }">
			<div class="content-wrap">
				...
				<p>
					您确知任何欺诈、违约行为均要承担相应的法律责任，您将本着诚实、信用的原则，自愿签订并依约履行相关协议。
				</p>
				<p>
				...
				<p>
					您已详细咨询并完全理解利息及相关费用的收取规则、禁止行为、违约后果等。相关协议一经您勾选并提交即视为您已完全理解并同意协议全部条款，且视为您已签署相关协议并将在协议生效后承担相应的法律责任。
				</p>
				<div class="signature">
...
export default {
	...
	methods: {
			...
			Reminder.appCFCA(appCFCAObj).then(res => {
				...
				if (data.resultCode === "1") {
					if (this.utils.getCookie("backUrl")) {
						...
						this.common.updateChannelApplicationStatusWacai(wacaiObj).then(res => {
		...
		calculateLoanPlan() {
			...
			this.common.calculateLoanPlan().then(data => {})
		}
	},
	mounted() {
