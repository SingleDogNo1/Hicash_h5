<template>
  <div class="content">
    ...
    <confirm :class="{ 'haiqian' : themes === 'haiqian'}" v-model="showSinaPayPop" title="提示" @on-confirm="ifJumpSinaPay(jumpSinaPayData)" @showDialog="showDialog" confirm-text="取消" cancel-text="确定">
      <p style="text-align:center;" class="dialog-content">{{dialogMsg}}</p>
    </confirm>
  </div>
</template>
  ...
  /deep/ .vux-confirm {
    .weui-dialog {
      ...
      .weui-dialog__bd {
        ...
        //padding: 0;
        .dialog-content {
          padding: 0 rem(12px) rem(32px) rem(12px);
          margin-top: rem(4px);
          text-align: center;
          font-size: 13px;
          color: #333333;
...
export default {
  ...
  methods: {
    showDialog(showDialog) {
      this.showAbleModifyName = showDialog;
      this.showEnableModifyName = showDialog;
      this.showSinaPayPop = showDialog;
    },
    goBack() {},
    close() {},
    showPop() {
    ...
    checkCard() {
      ...
      cardCheckFourObj.append("userName", this.userName);
    ...
    submitOrder() {
      ...
      var comeFrom = this.utils.getPlatform();
    ...
    createOrder() {
      ...
      baiQiShiObj.append("name", encodeURIComponent(unescape(utils.getCookie("realName"))));
      baiQiShiObj.append("eventType", "binding");
      baiQiShiObj.append("tokenKey", utils.getCookie("uuid"));
      baiQiShiObj.append("plateform", utils.getApplyFrom());
                ...
                if (status) {
                  ...
                  common
                    ...
                    .then(res => {
                      ...
                      this.$router.push({ name: "loanProgress"})
                      // window.location.href =
