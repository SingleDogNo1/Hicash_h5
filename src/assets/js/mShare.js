;(function(global, factory) {
					...
					types.forEach(function(item) {
						var _info = {
							title:
								...
								config.title,
							desc:
								(config.infoMap && config.infoMap[item] && config.infoMap[item].desc) ||
								config.desc,
							link:
								(config.infoMap && config.infoMap[item] && config.infoMap[item].link) ||
								config.link,
							imgUrl:
								(config.infoMap &&
									config.infoMap[item] &&
	...
	var getDefaultConfig = function getDefaultConfig(config) {
		...
		return {
			...
			infoMap:
				infoMapType === "function" || (infoMapType === "object" && !!config.infoMap)
					? config.infoMap
					: {}
		}
	}
