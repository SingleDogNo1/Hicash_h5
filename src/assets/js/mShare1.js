...
;(function() {
	...
	function createHtml() {
		var sHtml =
			...
			'          <a href="' +
			defaults.url +
			'" target="_blank" class="cory-cont-link">' +
			defaults.url +
			"</a>" +
			"      </span>" +
			"  </div>" +
	...
	function checkShareAction(target) {
		...
		if (isucBrowser) {
			...
			switch (id) {
				...
				case "kQZone":
					var qzLink =
						"http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?site=www.tuan2.com&title=" +
						defaults.title +
						"&desc=" +
						defaults.desc +
						"&summary=" +
	...
	function ucShare(toApp) {
		if (toApp == "QZone") {
			...
			;(k = document.createElement("div")),
				...
				(k.innerHTML =
					'<iframe src="' + B + '" scrolling="no" width="1" height="1"></iframe>'),
	...
	var api = {
		init: function(config) {
			...
			defaults.img =
				...
				""
			defaults.desc =
				config.desc || document.getElementsByName("description") || ""
