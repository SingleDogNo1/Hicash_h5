    ...
    <div class="rate-calculation-body">
        ...
        <section class="base-main">
            <div class="person-content">
                <label>直系亲属姓名</label>
                <x-input v-model="directRelativesName" name="directRelativesName" placeholder="请输入真实姓名" :show-clear="false">
                </x-input>
            </div>
            <div class="person-content">
                <label>关系</label>
                <popup-picker :data="directRelativesRelationList" v-model="currentDirectRelativesRelation" value-text-align="right" :columns="4" @on-change="directRelativesRelationChange" show-name>
                </popup-picker>
            </div>
            <div class="person-content">
                <label>直系亲属手机号</label>
                <x-input v-model="directRelativesTel" name="directRelativesTel" placeholder="请输入手机号" :show-clear="false">
                </x-input>
            </div>
        </section>
        <section class="base-main stu-contact">
            <div class="person-content">
                <label>紧急联系人姓名</label>
                <x-input v-model="emergencyContactName" name="emergencyContactName" placeholder="请输入真实姓名" :show-clear="false">
                </x-input>
            </div>
            <div class="person-content">
                <label>关系</label>
                <popup-picker :data="emergencyContactRelationList" v-model="currentEmergencyContactRelation" value-text-align="right" :columns="4" @on-change="emergencyContactRelationChange" show-name>
                </popup-picker>
            </div>
            <div class="person-content">
                <label>紧急联系人手机</label>
                <x-input v-model="emergencyContactTel" name="emergencyContactTel" placeholder="请输入手机号" :show-clear="false">
                </x-input>
            </div>
        </section>
        <section class="base-main stu-contact" v-if="isShowSpouseInfo">
            <div class="person-content">
                <label>配偶姓名</label>
                <x-input v-model="spouseName" name="spouseName" placeholder="请输入真实姓名" :show-clear="false">
                </x-input>
            </div>
            <div class="person-content">
                <label>配偶手机号</label>
                <x-input v-model="spouseTel" name="spouseTel" placeholder="请输入手机号" :show-clear="false">
                </x-input>
            </div>
        </section>
        <div class="next-step">
    ...
    export default {
        ...
        methods : {
            ...
            btnNext () {
                ...
                if(this.currentDirectRelativesRelation === '') {
                ...
                } else if (this.directRelativesName === this.emergencyContactName || this.directRelativesName === userRealName || this.emergencyContactName === userRealName) {
                    errorMsg = '直系亲属姓名，紧急联系人姓名，本人姓名不能相同';
                } else if (this.currentDirectRelativesRelation === this.currentEmergencyContactRelation) {
                    errorMsg = '直系亲属关系和紧急联系人关系不能相同';
                } else if (this.directRelativesTel === this.emergencyContactTel || this.directRelativesTel === userMobile || this.emergencyContactTel === userMobile) {
                    ...
                    if(errorMsg == ""){
                        if(this.spouseName === this.directRelativesName || this.spouseName === this.emergencyContactName || this.directRelativesName === this.emergencyContactName || this.directRelativesName === userRealName || this.emergencyContactName === userRealName || this.spouseName === userRealName) {
                            errorMsg = '配偶姓名，直系亲属姓名，紧急联系人姓名，本人姓名不能相同';
                        } else if (this.spouseTel === this.directRelativesTel || this.spouseTel === this.emergencyContactTel || this.directRelativesTel === this.emergencyContactTel || this.directRelativesTel === userMobile || this.emergencyContactTel === userMobile || this.spouseTel === userMobile) {
                            errorMsg = '配偶手机号码，直系亲属手机号码，紧急联系人手机号码，本人手机号码不能相同';
                        }
                    }
                }
                if(errorMsg !== ""){
                ...
                let postData = new URLSearchParams();
                    postData.append('userName', utils
                        .getCookie("userName"));
                    ...
                    common.updateRelationInfo(postData)
                        .then((res)=>{
                            if(res.data.resultCode === "1"){
                                ...
                                common.queryShouQuanSuc(params)
                                    .then((result)=>{
                                        if(result.data.resultCode === "1"){
                                            ...
                                            common.updateTempAppInfo(postObj)
                                                .then((res)=>{
                                                });
        ...
        mounted: function () {
            ...
            let postData = new URLSearchParams();
                ...
                common.relationInfo(postData)
                    .then((res)=>{
                        this.directRelativesName = res.data.immediate_name;
                        let currentDirectRelativesRelation = this.directRelativesRelationList.filter((item) => {
                        ...
                        let currentEmergencyContactRelation = this.emergencyContactRelationList.filter((item) => {
