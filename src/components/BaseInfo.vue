    ...
    <div class="rate-calculation-body">
        ...
        <section class="base-main">
            <div class="person-content">
                <label>姓名</label>
                <x-input v-model="userName" name="userName" placeholder="请输入真实姓名" :disabled="isDisabled" :show-clear="false">
                </x-input>
            </div>
            <div class="person-content">
                <label>身份证号码</label>
                <x-input v-model="idCard" name="idCard" placeholder="请输入身份证号码" :disabled="isDisabled" :show-clear="false">
                </x-input>
            </div>
            <div class="person-content">
                <label>婚姻状况</label>
                <popup-picker :data="maritalStatusList" v-model="currentMaritalStatus" value-text-align="right" :columns="4" @on-change="maritalStatusChange" show-name>
                </popup-picker>
            </div>
            <div class="person-content">
                <label>最高学历</label>
                <popup-picker :data="highestEducationList" v-model="currentHighestEducation" value-text-align="right" :columns="5" @on-change="highestEducationChange" show-name>
                </popup-picker>
            </div>
            <div class="person-content">
                <label>现居地址</label>
                ...
                <router-link class="address" :to="{name: 'Address', params: {from: 'address'}}">
                    {{address}}
                </router-link>
            ...
            <div class="person-content">
                <label>现居详细地址</label>
                <x-input v-model="detailAddress" name="detailAddress" placeholder="请输入详细地址" :show-clear="false">
                </x-input>
            </div>
            <div class="person-content">
                <label>电子邮箱</label>
                <x-input v-model="email" name="userName" placeholder="请输入邮箱" type="email" required :show-clear="false">
                </x-input>
            </div>
            <div class="person-content">
                <label>借款用途</label>
                <popup-picker :data="loanPurposeList" v-model="currentLoanPurpose" value-text-align="right" :columns="10" @on-change="loanPurposeChange" show-name>
                </popup-picker>
            </div>
        </section>
        <!-- 单位信息 -->
        ...
        <section class="base-main">
            <div class="person-content">
                <label>单位名称</label>
                <x-input v-model="companyName" name="companyName" placeholder="请输入单位名称" :show-clear="false">
                </x-input>
            </div>
            <div class="person-content company-tel">
                <label>单位电话（需填写区号）</label>
                <x-input v-model="companyTel" name="companyTel" placeholder="示例：021-51234567" :show-clear="false">
                </x-input>
            </div>
            <div class="person-content">
                <label>工作年限</label>
                <popup-picker :data="workYearList" v-model="currentWorkYear" value-text-align="right" :columns="4" @on-change="workYearChange" show-name>
                </popup-picker>
            </div>
            <div class="person-content">
                <label>单位行业</label>
                <popup-picker :data="companyIndustryList" v-model="currentCompanyIndustry" value-text-align="right" :columns="13" @on-change="companyIndustryChange" show-name>
                </popup-picker>
            </div>
            <div class="person-content">
                <label>单位地址</label>
                ...
                <router-link class="address" :to="{name: 'Address', params: {from: 'companyAddress'}}">
                    {{companyAddress}}
                </router-link>
                <x-icon type="ios-arrow-right" class="right-inco" size="34"></x-icon>
            </div>
            <div class="person-content">
                <label>单位详细地址</label>
                <x-input v-model="companyDetailAddress" name="companyDetailAddress" placeholder="请输入详细地址" :show-clear="false">
                </x-input>
            </div>
        </section>
        <div class="next-step">
    ...
    export default {
        ...
        methods : {
            ...
            btnNext () {
                ...
                common.updateCollarPersonInfo(postData)
                    .then((res)=>{
                        if(res.data.resultCode === "1") {
                            utils.setCookie("realName", escape(this.userName));
                            utils.setCookie("identityCode", this.idCard);
                            utils.setCookie("homeAddress",escape(this.address.replace("/","").replace("/","") + this.detailAddress));
                            utils.setCookie("workAddress",escape(this.companyAddress.replace("/","").replace("/","") + this.companyDetailAddress));
                            ...
                            if(source == "APP"){
                                ...
                                this.$router.push({path:"/login", query: {redirect: _this.$route.fullPath, source: 'APP'}});
                            } else {
                                this.$router.push({path:"/login", query: {redirect: _this.$route.fullPath}});
                            }
                        } else {
        ...
        mounted: function () {
            ...
            common.collarPersonInfo(params)
                .then((res)=>{
                    if(res.data.resultCode === "1") {
                        if(!res.data.real_name || !res.data.id_no || res.data.real_name == "" || res.data.id_no == ""){
                            ...
                            if(res.data.province && res.data.province_name && res.data.city && res.data.city_name && res.data.area && res.data.area_name){
                                this.address = res.data.province_name + "/" + res.data.city_name + "/" + res.data.area_name;
                                this.province = res.data.province;
                                this.city = res.data.city;
                                this.area = res.data.area;
                            }
                            if(res.data.unit_province && res.data.unit_province_name && res.data.unit_city && res.data.unit_city_name&& res.data.unit_area && res.data.unit_area_name){
                                this.companyAddress = res.data.unit_province_name + "/" + res.data.unit_city_name + "/" + res.data.unit_area_name;
